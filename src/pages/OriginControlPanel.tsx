...
const OriginControlPanel = () => {
  ...
  useEffect(() => {
    ...
    const loadSubmissions = async () => {
      ...
      const { data, error: fetchError } = await supabase
        .from("submissions")
        .select(
          "teamID, Team_Name, Problem_Statement, Repo_URL, Progress, Total_Scores, Tech_Scores, Innov_Scores, Completeness_Scores, password"
        )
        .limit(500);
      ...
      setSubmissions(
        (data || []).map((row) =>
          toSubmissionRecord(row as Record<string, unknown>)
        )
      );
      setLoading(false);
    };
  ...
  const stats = useMemo(() => {
    ...
    const completed = submissions.filter(
      (row) => row.Progress.toLowerCase() === "completed"
    ).length;
    const queued = submissions.filter(
      (row) => row.Progress.toLowerCase() === "queued"
    ).length;
  ...
  const sortedSubmissions = useMemo(() => {
    ...
    const compareProblem = (
      ...
      const lp = left.Problem_Statement.trim().toLowerCase();
      const rp = right.Problem_Statement.trim().toLowerCase();
    ...
    rows.sort((left, right) => {
      if (problemSort !== "none" && marksSort !== "none") {
        const pc = compareProblem(left, right);
        if (pc !== 0) return pc;
  ...
  const saveEdit = async () => {
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        Progress:
          editForm.Progress.trim().toLowerCase() === "completed"
            ? "completed"
            : "queued",
  ...
  const addRow = async () => {
    ...
    const { error: insertError } = await supabase
      ...
      .insert({
        ...
        Progress:
          newForm.Progress.trim().toLowerCase() === "completed"
            ? "completed"
            : "queued",
      ...
      <div className="mx-auto w-full max-w-[1400px] space-y-8">
        ...
        >
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Edit, add, and manage every submissions field directly.
          </p>
          <div className="mt-4 flex gap-3">
            <button
              onClick={() => navigate("/orehackproject1924")}
              className="rounded-lg border border-border px-3 py-1.5 text-xs font-semibold text-foreground hover:bg-card"
            >
              ← Back to Dashboard
            </button>
          </div>
          ...
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <select
                value={marksSort}
                onChange={(e) =>
                  setMarksSort(e.target.value as typeof marksSort)
                }
              ...
              <select
                value={problemSort}
                onChange={(e) =>
                  setProblemSort(e.target.value as typeof problemSort)
                }
                className="rounded-lg border border-border bg-background px-3 py-2 text-sm"
              >
                ...
                {!loading &&
                  sortedSubmissions.map((row) => {
                    ...
                    return (
                      <tr
                        key={row.teamID}
                        className="border-b border-border/60 text-sm hover:bg-card/60"
                      >
                        <td className="px-4 py-3">
                          {isEditing ? (
                            <input
                              ...
                              onChange={(e) =>
                                onEditChange("teamID", e.target.value)
                              }
                          ...
                          >
                            {isEditing ? (
                              <input
                                ...
                                onChange={(e) =>
                                  onEditChange("Team_Name", e.target.value)
                                }
                                className="w-full rounded border border-border bg-background px-2 py-1"
                              />
                            ) : (
                          ...
                          >
                            {isEditing ? (
                              <input
                                ...
                                onChange={(e) =>
                                  onEditChange("Repo_URL", e.target.value)
                                }
                                className="w-full rounded border border-border bg-background px-2 py-1"
                              />
                            ) : (
                        ...
                        <td className="px-4 py-3">
                          {isEditing ? (
                            <select
                              ...
                              onChange={(e) =>
                                onEditChange("Progress", e.target.value)
                              }
                              className="w-full rounded border border-border bg-background px-2 py-1"
                            >
                              <option value="queued">queued</option>
                        ...
                        <td className="px-4 py-3 text-right font-semibold text-foreground">
                          {isEditing ? (
                            <input
                              ...
                              onChange={(e) =>
                                onEditChange("Total_Scores", e.target.value)
                              }
                        ...
                        <td className="px-4 py-3">
                          <div
                            className="w-[120px] truncate font-mono text-[10px]"
                            title={row.password}
                          >
                            {isEditing ? (
                              <input
                                ...
                                onChange={(e) =>
                                  onEditChange("password", e.target.value)
                                }
                                className="w-full rounded border border-border bg-background px-2 py-1"
                              />
                            ) : (
                        ...
                        <td className="px-4 py-3">
                          {isEditing ? (
                            ...
                            <div className="flex items-center justify-between gap-3 group/problem">
                              ...
                              <button
                                onClick={() =>
                                  setViewingProblem(row.Problem_Statement)
                                }
