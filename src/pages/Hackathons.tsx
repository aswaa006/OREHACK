...
/* ══════════════════════════════════════════════════════════
   DATA
   (Keep same data as ActiveHackathons)
═══════════════════════════════════════════════════════════ */
const hackathons = [
...
/* ══════════════════════════════════════════════════════════
   COMPONENTS (Sparkline, ExplodingCard, StatRow)
═══════════════════════════════════════════════════════════ */
  ...
  return (
    <div className="min-h-screen bg-[#080c14] text-foreground">
      ...
      <style>{`
        ...
        @keyframes borderBreath { 0%,100% { opacity: 1; } 50% { opacity: 0.45; } }
        ...
        .dot-btn:hover .dot-label { opacity: 1 !important; max-width: 160px !important; margin-left: 0.55rem; }
        .dot-label { max-width: 0; opacity: 0; overflow: hidden; white-space: nowrap; transition: all 0.3s ease; }
      `}</style>

      <div ref={trackRef} style={{ height: `${TRACK_VH * 100}vh`, position: "relative" }}>
        <div style={{
          ...
          <div style={{ width: "100%", position: "relative", zIndex: 1 }}>
            <div style={{
              ...
              <p style={{ fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "hsl(217 91% 60%)", marginBottom: "0.6rem" }}>
                ◆ Active Hackathons
              </p>
              <h2 style={{
                ...
                Compete.<br />
                <span style={{ background: "linear-gradient(135deg, hsl(217 91% 60%), hsl(263 84% 68%))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                  Conquer.
                </span>
              </h2>
                ...
                {hackathons.map((h, i) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ display: "flex", alignItems: "center", background: "none", border: "none", cursor: "pointer", padding: 0, textAlign: "left" }}
                    >
                      <div style={{
                        width: isActive ? 30 : 8, height: 8, borderRadius: 9999,
                        background: isActive ? sc.card : "hsl(217 33% 26%)", flexShrink: 0,
                      ...
                      <span className="dot-label" style={{ fontSize: "0.75rem", fontWeight: isActive ? 700 : 500, color: isActive ? sc.card : (hoveredDot === i ? "hsl(220 14% 80%)" : "hsl(218 11% 40%)") }}>
                        {h.name}
                      </span>
                    </button>
                  );
            ...
            <div style={{ marginLeft: "clamp(280px, 36vw, 420px)", padding: "4rem 0", marginTop: "-4rem", overflow: "hidden", maskImage: "linear-gradient(90deg, black 0%, black 90%, transparent 100%)" }}>
              <div ref={railRef} style={{ display: "flex", gap: "2rem", paddingRight: "clamp(2rem, 5vw, 6rem)", willChange: "transform", perspective: "1000px" }}>
                {hackathons.map((h, i) => (
                  <div key={h.id} style={{ width: "clamp(300px, 38vw, 480px)", height: "clamp(280px, 46vh, 380px)", flexShrink: 0 }}>
            ...
            <div style={{ position: "absolute", bottom: "-4rem", left: "clamp(1.5rem, 5vw, 4rem)", right: "clamp(1rem, 3vw, 3rem)", height: 2, background: "hsl(217 33% 18%)", borderRadius: 9999, overflow: "hidden" }}>
              <div ref={progressBarRef} style={{ height: "100%", width: "0%", borderRadius: 9999, background: "linear-gradient(90deg, hsl(217 91% 60%), hsl(263 84% 68%))", transition: "width 0.08s linear" }} />
            </div>
          </div>
        </div>
      </div>
